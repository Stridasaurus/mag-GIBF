...
> **Status:** active — **Card A Tier 1 COMPLETE (2026‑07‑06)**: §8‑iv signed off (floor amendment; §8‑viii MDL pinned) and the Tier‑1 spike run the same day (co‑primary thresholds pinned at 0.394/0.137; fail arm gap‑conditioned; exploitable band reachable at IMAGE spacings for mid‑range Q·β). Frontier is now **Gate V** (the `secsy` CF‑pair probe — first build action). Manifesto #1 (Viability Test & Methods Paper) is being expanded now from **Stub 0**.
...
- **Active node: Gate V (first build action). Card A Tier 1 is COMPLETE (2026‑07‑06, both steps same day).** §8‑iv signed off with the floor amendment and §8‑viii (MDL) pinned in the morning sitting; the Tier‑1 spike ran the same session (`viability-test/tier1_flr_coherence.py`, results + figures in `results/A_flr_coherence/` — see the §8 result entry). Headlines: co‑primary thresholds pinned (0.394 / 0.137, both informative at N=64); the incoherent κ floor is degeneracy‑dominated (0.28, N‑independent) while the φ=0 coherent floor is clean (0.012), so the fail arm is **gap‑conditioned**; FLR geometry confirms the exploitable band is reachable at IMAGE spacings for mid‑range (Q, β) — spacing‑in‑half‑widths is the controlling parameter. **H‑A not adjudicated** — that is Tier 2, which stays blocked until Gate V is green.
...
- **Open questions that gate airtightness (resolve before/at manifesto lock):**
  ...
  2. **Threshold sign‑offs:** §8‑iv **done 2026‑07‑06** (with floor amendment; §8‑viii MDL pinned same sitting); §8‑iii, ‑vi, ‑vii remain — before Experiment B runs.
