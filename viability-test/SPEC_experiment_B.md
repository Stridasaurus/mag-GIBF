...
## S.12 d=8 also degenerate; d=2 accepted as the confirmatory coordinate (2026‑08‑28)

**d=8's result.** The confirmatory pass at `d=8` (`run_experiment_B2B6_d8.py`, branch `b2b6-confirmatory-d8` / PR #4) came back **more decisively degenerate than `d=3`**: the B1 matched cell and the coherent mini‑pilot both returned gap SD = 0.0000 exactly, 50/50 trials, all three solvers (L2, GIBF, MMV) exact‑recovering — P_sep = 1.00 across the board. The pre‑registered win and loss rules are structurally unsatisfiable at zero variance, so the §8‑ii power calc was not evaluated and powered B2/B6a were correctly not built. `d=8` is **not** re‑picked — it stands exactly as drawn (§S.11), reported as a ceiling finding in its own right.

**The meta‑finding that actually settles the next step.** Reading the already‑committed 2026‑07‑27 B1 incoherent‑sweep grid (existing data, no new run) shows `d=5` sits at the identical ceiling across every SNR from −5 to 20 dB, and `d=1` sits at the opposite floor. Of the four candidates in the §S.11 blind‑draw pool `{1,2,5,8}`, **only `d=2` was ever genuinely uncertain** — the sole outcome not already knowable from data that existed before the draw. The draw mechanism itself was unbiased (CSPRNG, no result consulted); the pool it drew from was not informationally neutral, since three of its four outcomes were already determined by data on hand. This is reported as a design observation, not grounds to redraw — no result was used to influence or avoid the original draw.

**Strider's ruling (2026‑08‑16, reported at the team restart meeting, 2026‑08‑28): accept `d=2` directly**, on exactly this basis — the cheapest path forward, and not a second blind draw (there is nothing left in the pool to draw *from* that isn't already determined). `φ=90°`, `snr_db ∈ {5,10}`, `n_snap=64` stay pinned unchanged; only the confirmatory `d` moves from 8 to **2** (127 km, §S.10's `d` axis).

**Result (2026‑08‑28, `run_experiment_B2B6_d2.py`, branch `b2b6-confirmatory-d2`): ADJUDICABLE.** Neither solver saturates on every trial (`at_ceiling=[]`, `at_floor=[]`); P_sep spreads across methods (L2 0.00, GIBF 0.74, MMV 0.80) rather than pinning at 1.00. The §8‑ii power calc produced a real number for the first time in this project: variance input 8.3066 cells (paired signed‑gap SD, consistent with the historical B1 `d=2`/5dB reading of 8.1819), effect size 0.7375 cells (20% of the MMV baseline, the conservative arm of the OR rule), **n_trials = 1635** at α=0.006, power=0.80. Pilot‑scale gap signs disagree between the B1 cell (favors GIBF) and the mini‑pilot (favors MMV) — expected noise at n=50, not a preview of the powered verdict.

**Next.** Build the powered B2/B6 runner (§S.2‑B2 win/null cells, §S.6.5 regularization‑sensitivity panel, §S.3 B6a paired oracle/K̂ arm) at `d=2` — no confirmatory coordinate has reached this step before, so this is new code, not a parameter swap on an existing runner. Estimated compute: ~1 minute/cell at n=1635 (100 trials ran in 3.4s in the pilot pass).

## S.13 Grid‑density / reduction‑threshold interaction, characterized (2026‑08‑28)

**Origin.** An abandoned, never‑merged exploratory branch (`geometry-rescale-sonnet`, 2026‑07‑27, superseded by the actual §S.10 33×17 rebuild) flagged but did not fix a confound: `grid_reduction`'s stopping rule (`len(cols) < min_active_factor × n_channels`, §S.6(1)) is an **absolute** threshold, not one that scales with grid density — so a denser pole grid mechanically needs more IRLS iterations before reduction can trigger. That branch measured 7 iterations at a 17×17 (289‑pole) grid vs 1 at the archived‑brief 11×11 (121‑pole) grid — and explicitly declined to fix it, since doing so would mean touching the pinned §S.6(1) config without authorization. The finding was never carried into this file or `ROADMAP.md`, and the actual shipped grid (33×17, 561 poles, §S.10) is denser still — so the question was open, not resolved, until this section.

**Mechanism, confirmed directly from `solvers.py`.** Column pruning under `grid_reduction` is deterministic: each iteration keeps `floor(β × current_count)`, independent of data. At the current grid (`n_poles=561`, `n_channels=75`, `min_active_factor=2` ⇒ threshold `150`, `β=0.9`), the geometric decay from 561 crosses below 150 at iteration **13**, exactly — a fixed property of the pinned constants, not of the physics or the trial.

**Empirical check (2026‑08‑28, 15 trials, `d=2`, φ=90°, 5dB, coherent, reduction ON — the confirmatory setting):**

| | GIBF (per mode) | MMV |
|---|---|---|
| n_iter | 13 on most trials; 7–8 on a few (own tol check fires first) | **exactly 13 on all 15 trials** |
