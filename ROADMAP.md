...
> **Status:** active — frontier is **Experiment A, Tier 1** (FLR source‑coherence forward model): §8‑iv **signed off 2026‑07‑06** (floor amendment; §8‑viii MDL pinned); the Tier‑1 spike itself is the next action. Manifesto #1 (Viability Test & Methods Paper) is being expanded now from **Stub 0**.
...
- **Active node: A (Card A, Tier 1 — FLR source‑coherence forward model) — Step 1 DONE (2026‑07‑06), Step 2 unblocked.** **Step 1 (done):** §8‑iv signed off with the floor amendment (N=64 pre‑registered, incoherent + φ=0 floor controls, floor‑referenced fail arm — see the §8 2026‑07‑06 entry); §8‑viii pinned **MDL** as B6a's solver‑fed `K̂` estimator at the same sitting. Thresholds were confirmed *before* the first number existed — the pre‑registration discipline held. **Step 2 (next action):** run the Tier‑1 spike (pure NumPy, zero build dependency): the `R(x)` profile, `φ_ij` vs pole spacing and `Q`, the κ / `‖Im S‖/‖S‖` calibration curves vs φ at N=64 (N=1024 secondary), plus the two floor controls. Do **not** run Tier 2 until Gate V is green.
